...
lint: ## flake8 + import-linter + mypy
	$(PY) -m flake8 .
	$(LINT_IMPORTS)
	@MYPY_COUNT=$$($(PY) -m mypy backend/ --ignore-missing-imports --no-error-summary 2>&1 | grep -c "^backend/" || true); \
	MYPY_MAX=$${SWARM_MYPY_MAX:-20}; \
	if [ $$MYPY_COUNT -gt $$MYPY_MAX ]; then \
		$(PY) -m mypy backend/ --ignore-missing-imports --no-error-summary 2>&1 | tail -20; \
...
test: ## pytest (excludes smoke tests)
	$(PY) -m pytest --maxfail=1 --disable-warnings -q --ignore=tests/smoke
