...
def _policy_keywords() -> dict[str, list[str]]:
    raw_policy = load_role_model_policy()
    raw = raw_policy.get("local_keyword_preferences") or {}
    if not isinstance(raw, dict):
        return {}
    groups = raw_policy.get("keyword_preference_groups") or {}
    if not isinstance(groups, dict):
        groups = {}

    def resolve(values: object) -> list[str]:
        if isinstance(values, str) and values.startswith("@"):
            g = groups.get(values[1:])
            if isinstance(g, list):
                return [str(item).strip().lower() for item in g if str(item).strip()]
            return []
        if isinstance(values, list):
            return [str(item).strip().lower() for item in values if str(item).strip()]
        return []

    return {str(role).strip(): resolve(values) for role, values in raw.items()}
...
def _cloud_default(provider: str, role: str) -> tuple[str, str]:
    ...
    if isinstance(provider_defaults, dict):
        catalog = provider_defaults.get("catalog")
        role_keys = provider_defaults.get("role_catalog_key")
        if isinstance(catalog, dict) and isinstance(role_keys, dict):
            slot = str(role_keys.get(role) or "").strip()
            if slot:
                model_id = str(catalog.get(slot) or "").strip()
        if not model_id:
