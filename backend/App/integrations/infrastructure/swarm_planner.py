...
def _valid_step_ids() -> list[str]:
    from backend.App.orchestration.application.routing.pipeline_graph import DEFAULT_PIPELINE_STEP_IDS

    return list(DEFAULT_PIPELINE_STEP_IDS)


def _remote_client_kwargs(ac: dict[str, Any], role_key: str) -> dict[str, Any]:
    from backend.App.orchestration.application.routing.pipeline_graph import _remote_api_client_kwargs_for_role
...
def _remote_kw(ac: dict[str, Any], role_key: str) -> dict[str, Any]:
    return _remote_client_kwargs(ac, role_key)
...
def _planner_remote_kw(
    ...
    from backend.App.orchestration.application.routing.pipeline_graph import _remote_api_client_kwargs
...
def plan_pipeline_steps(
    ...
    *,
    ...
    remote_kwargs = _planner_remote_kw(ac, planner_cfg)
    agent = BaseAgent(
        role="PM",
    ...
    raw = agent.run(user_message).strip()
