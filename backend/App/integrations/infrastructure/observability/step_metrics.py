"""Лёгкие in-process метрики шагов пайплайна (без обязательных зависимостей).

Счётчики и гистограммы по ``step_id``; опционально корреляция с ``task_id``.
Сброс при рестарте процесса. Для прод — включите OTEL экспорт.
"""
...
# Накопленные токены по step_id (суммируются по всем вызовам)
...
def _extract_token_metrics(delta: Mapping[str, Any]) -> dict[str, Any]:
    """Извлечь метрики токенов из step_delta, если они там есть."""
...
def record_step(
    ...
    *,
    ...
    # Логируем метрики токенов если есть хоть что-то ненулевое
