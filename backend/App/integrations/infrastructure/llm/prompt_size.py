"""Оценка размера chat-запроса к LLM (символы и грубый диапазон токенов)."""
...
class ChatRequestSize:
    """Сырые метрики; токены — ориентир (токенизатор модели может отличаться)."""
...
def estimate_chat_request_size(messages: list[dict[str, str]]) -> ChatRequestSize:
    ...
    # EN ~4 chars/token; RU/CJK/код часто плотнее — до ~2; даём диапазон.
...
def format_size_hint_ru(size: ChatRequestSize, model: str) -> str:
    part_a = (
        f"Размер запроса (оценка): ~{size.chars_total:,} символов UTF-8, "
        f"~{size.bytes_utf8:,} байт; токены ориентировочно "
        f"{size.approx_tokens_lo:,}–{size.approx_tokens_hi:,} "
        f"(середина ~{size.approx_tokens_mid:,}). "
        f"По ролям: system {size.chars_system:,}, user {size.chars_user:,}, "
        f"прочее {size.chars_other:,}. model={model!r}."
    )
    part_b = (
        "Как уменьшить: сократить system (prompts/), не вставлять целые документы в user; "
        "длинное — в файлы + краткая ссылка; меньше текста или модель с большим контекстом; "
        "после Channel Error в LM Studio — перезапуск/выгрузка модели."
    )
...
def maybe_warn_context_limit(model: str, size: ChatRequestSize) -> None:
    ...
    if limit <= 0:
        return
    # Запас под ответ и служебные токены чата
    ...
    if size.approx_tokens_hi > safe:
        logger.warning(
            "Промпт может не влезть в контекст: верхняя оценка токенов %s > лимит−запас %s "
