"""Подстановка текстов skills (markdown) из workspace в system prompt ролей."""
...
def _resolve_skill_file(workspace_root: str, rel_path: str) -> Optional[Path]:
    """Путь внутри workspace или, если root пуст, под каталогом prompts репозитория."""
    ...
    # Resolution: overrides → upstream → direct
...
def _catalog_by_normalized_keys(raw: dict[str, Any]) -> dict[str, Any]:
    """Ключи каталога приводим как id ролей (регистр, пробелы → _)."""
    out: dict[str, Any] = {}
...
def format_role_skills_extra(
    ...
    *,
    ...
    """Текст для ``BaseAgent.system_prompt_extra``: заголовки + тела skill-файлов."""
