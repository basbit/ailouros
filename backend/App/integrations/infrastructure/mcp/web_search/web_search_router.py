...
def _configured_keys(config_keys: Optional[dict[str, str]] = None) -> dict[str, str]:
    ...
    Priority (highest → lowest):
      1. Per-request config_keys (from UI agent_config.swarm / project settings).
      2. Environment variables SWARM_*_API_KEY (legacy / server-side config).
    """
    result: dict[str, str] = {}
    for provider in _PROVIDERS:
        env_var = _ENV_KEYS[provider]
        key = ((config_keys or {}).get(provider) or os.getenv(env_var, "") or "").strip()
        if key:
            result[provider] = key
    return result
