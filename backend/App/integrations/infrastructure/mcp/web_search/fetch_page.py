"""Fetch page tool — download a URL and extract readable text/markdown.

Builtin tool (no MCP server needed). Injected into tool list when
SWARM_FETCH_PAGE=1 (or agent_config.swarm.fetch_page=true).

Strict limits:
- Timeout: 10 seconds (configurable via SWARM_FETCH_PAGE_TIMEOUT)
- Max response size: 500 KB (configurable via SWARM_FETCH_PAGE_MAX_BYTES)
- Only HTTP/HTTPS URLs
- No private/internal IPs by default
"""
...
def fetch_page_available() -> bool:
    """Check if httpx is installed (it's in requirements.txt)."""
    try:
...
class _HTMLTextExtractor(HTMLParser):
    """Minimal HTML→text extractor. Strips tags, keeps text content."""
...
def fetch_page(url: str) -> str:
    ...
    Returns error string (not exception) on failure for safe tool dispatch.
    """
    import httpx
...
def fetch_page_tool_definition() -> dict[str, Any]:
    """Return an OpenAI-compatible tool definition for fetch_page."""
    return {
        "type": "function",
        "function": {
            "name": "fetch_page",
