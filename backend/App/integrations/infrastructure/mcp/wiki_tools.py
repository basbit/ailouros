"""Wiki MCP tools — search, read, and write wiki articles from agent tool calls.

Enabled when SWARM_WIKI_TOOLS=1.  The wiki root is resolved as:
    <workspace_root>/.swarm/wiki/
...
def wiki_tools_available(workspace_root: str) -> bool:
    """Return True when SWARM_WIKI_TOOLS=1 and the wiki root directory exists."""
    # env: SWARM_WIKI_TOOLS
...
def wiki_tools_definitions() -> list[dict[str, Any]]:
    """Return OpenAI-format tool schemas for the three wiki tools."""
    return [
        {
            "type": "function",
            "function": {
...
def _safe_wiki_root(workspace_root: str) -> Path:
    """Resolve and validate the wiki root; raise ValueError when missing."""
...
def _sanitise_rel_path(rel_path: str) -> str:
    """Strip leading slashes and .md suffix; raise ValueError on traversal."""
...
def handle_wiki_tool_call(
    ...
    """Dispatch a wiki tool call and return a string result.

    Returns an error string (never raises) so callers can feed the result
    back into the tool-call loop without crashing.
    """
    try:
