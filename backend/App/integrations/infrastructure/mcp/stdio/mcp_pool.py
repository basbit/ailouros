"""MCP pool and config loaders.

Extracted from session.py: coerce_mcp_config_dict, load_mcp_server_defs,
MCPPool, mcp_preflight_check.
"""
...
# Tool names from @modelcontextprotocol/server-filesystem that only read state.
# Used when readonly=True in openai_tools() to reduce the tool-schema token overhead
# for small-context local models.  Planning roles (PM/BA/Arch) never write files, so
# they only need these tools.
...
def _mcp_compact_tools_enabled() -> bool:
    """Enable compact tool schema mode to save tokens on small-context models.
...
def _mcp_tool_description_max_chars() -> int:
    """Character limit for each tool description in compact mode (default 200).

    200 chars fits a single-line summary of what the tool does — enough for
    a small-context model to decide which tool to call.
    """
...
def coerce_mcp_config_dict(cfg: dict[str, Any]) -> dict[str, Any]:
    """Поддержка формата Cursor / Claude: верхний ключ ``mcpServers`` (map → servers)."""
...
def load_mcp_server_defs(cfg: Any) -> list[dict[str, Any]]:
    """cfg: dict с ключом servers или путь к JSON-файлу (str)."""
...
class MCPPool:
    """Несколько stdio-серверов; префикс имён инструментов: ``server__tool``."""
    ...
    def __init__(self, defs: list[dict[str, Any]], cancel_event: Optional[Any] = None) -> None:
        ...
        # BUG-F3: Extract workspace_root from the workspace server's args[0]
        self._workspace_root: str = ""
        for d in defs:
            if d.get("name") == "workspace":
                cmd = d.get("command") or []
                # The workspace root is the last CLI arg (after the binary path)
    ...
    def openai_tools(
        ...
        *,
        ...
        """Return OpenAI-compatible tool definitions from all MCP servers.

        readonly: when True only expose tools whose name is in _READONLY_TOOL_NAMES
            (read_file, list_directory, search_files, …).  Reduces tool-schema token
            overhead for small-context local models.  Override the set via
            SWARM_MCP_READONLY_TOOL_NAMES (comma-separated names).
        """
    ...
    def dispatch_tool(
        ...
        # BUG-F3: Normalize relative paths for workspace tools
        ...
        # P0-5: Reject absolute paths outside workspace_root
        if sname == "workspace" and self._workspace_root:
        ...
        # BUG-F8: Auto-create parent directories for write_file/edit_file
        # MCP write_file fails with "Parent directory does not exist" if dirs missing.
        # Pre-create them to avoid wasting tool rounds on create_directory calls.
...
def mcp_preflight_check(server_def: dict[str, Any]) -> dict[str, Any]:
    """Spawn one MCP server, do tools/list, then shut it down.

    Returns a result dict for the ``mcp_preflight`` SSE event:
    ``{ status: "ok"|"failed", phase: "spawn"|"tools_list", server: str, error: str|None }``
    """
    from backend.App.integrations.infrastructure.mcp.stdio.session import (
        MCPStdioSession,
        _mcp_init_timeout_sec,
    ...
    # Build full command list: combine command (str or list) + args (list).
    # This mirrors the logic in load_mcp_server_defs so the server receives
    # all required arguments (workspace path, npx package name, etc.).
    ...
    # Merge server-specific env over current process env (same as MCPPool does).
    ...
    try:
        ...
        # Broad exception: preflight check must never propagate — caller always
        # expects a result dict, not a raised exception.
