"""MCP поверх stdio: Content-Length framing и newline-delimited JSON (Model Context Protocol).

Протокол авто-определяется при handshake():
  • SDK ≥ v1.28 (@modelcontextprotocol/server-filesystem v2026+): JSON + '\\n'
  • Старые серверы (MCP SDK < v1.28): Content-Length: N\\r\\n\\r\\nJSON
Сначала пробуется JSON\\n (5s); при таймауте — откат на Content-Length.
"""
...
# Сентинел: stdout закрыт / reader завершился.
_MCP_RX_EOF = object()
...
def _mcp_init_timeout_sec() -> float:
    """Таймаут фазы инициализации MCP (spawn → initialize → tools/list).

    Отдельный от SWARM_MCP_RPC_WAIT_SEC — он для tool-call RPC во время работы.
    По умолчанию 60s; переопределить через SWARM_MCP_INIT_TIMEOUT_SEC.
    При истечении: явная ошибка, без тихого продолжения (INV-1).
    """
    try:
...
def _frame_message(payload: dict[str, Any]) -> bytes:
    """Content-Length framing (MCP SDK < v1.28)."""
...
def _frame_message_newline(payload: dict[str, Any]) -> bytes:
    """Newline-delimited JSON (MCP SDK ≥ v1.28, default for @mcp/server-filesystem v2026+)."""
    return (json.dumps(payload, ensure_ascii=False) + "\n").encode("utf-8")


def _read_one_message(stream) -> Optional[dict[str, Any]]:
    """Читает одно JSON-RPC сообщение с Content-Length.

    Возвращает None ТОЛЬКО при настоящем EOF (пустое чтение).
    Мусорные строки от npx/npm (прогресс, предупреждения) пропускаются —
    раньше они возвращали None и ложно сигнализировали EOF, вешая handshake
    на 600 секунд вместо немедленного фолбэка.
    """
    while True:
        ...
        if not line.lower().startswith(b"content-length:"):
            # Пробуем распарсить как голый JSON (некоторые серверы)
            try:
                return json.loads(line.decode("utf-8").strip())
            except json.JSONDecodeError:
                # Мусор (npm warn, npx progress, …) — пропускаем, продолжаем читать
        ...
        crlf = stream.read(2)
        if crlf != b"\r\n":
            # Нестандартный разделитель — пропускаем оставшиеся байты этого фрейма
            stream.read(max(0, n))
            continue
        raw = stream.read(n)
        if len(raw) != n:
            return None  # Усечённое чтение = настоящий EOF
        try:
...
class MCPStdioSession:
    """Один MCP-сервер как подпроцесс."""
    ...
    # Протокол: True → JSON\n (SDK ≥ v1.28); False → Content-Length (legacy).
    # Определяется автоматически при handshake().
    _newline_proto: bool = False

    @staticmethod
    def _drain_stderr_loop(proc: subprocess.Popen, tail: bytearray) -> None:
        """Снимает stderr в фоне; иначе PIPE забивается и дочерний процесс зависает навсегда.

        Последние 4 КБ сохраняются в ``tail`` для диагностики при EOF.
        """
        err = proc.stderr
        if err is None:
            return
    ...
    def start(self) -> None:
        if self._proc is not None:
            return
        # Всегда захватываем stderr — дрейним в фоне, чтобы PIPE не забился (deadlock),
        # и сохраняем последние 4 КБ в _stderr_tail для диагностики при EOF.
    ...
    def close(self) -> None:
        """Закрыть stdin → сигнал дочернему процессу; затем terminate/kill и закрыть stdout.

        Закрытие stdout со стороны родителя будит ``readline()`` в reader-потоке (см. issues
        python-sdk stdio hang на shutdown).
        """
    ...
    def _write_msg(self, msg: dict[str, Any]) -> None:
        """Отправить JSON-RPC сообщение в текущем wire-формате (newline / Content-Length)."""
    ...
    def request(
        ...
        # Buffer responses for other request ids so they are not lost permanently.
        _out_of_order: dict[int, dict] = {}
        while True:
            # Check buffer first (a previous iteration may have stashed our response).
            ...
            if cancel_event is not None and cancel_event.is_set():
                raise RuntimeError(f"MCP {self.name}: отменено (pipeline cancel)")
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                if _deadline_override is not None:
                    raise RuntimeError(
                        f"MCP {self.name}: таймаут инициализации (SWARM_MCP_INIT_TIMEOUT_SEC={int(_mcp_init_timeout_sec())}s) — "
                        "увеличьте через SWARM_MCP_INIT_TIMEOUT_SEC"
                    )
                raise RuntimeError(
                    f"MCP {self.name}: таймаут ожидания ответа (SWARM_MCP_RPC_WAIT_SEC)"
                )
            try:
            ...
            if resp is _MCP_RX_EOF:
                ...
                raise RuntimeError(
                    f"MCP {self.name}: EOF или битый ответ (reader). stderr: {err}"
                )
            # Server notifications (no "id") — skip; never raise on their "error" field.
            if "id" not in resp:
                continue
            if resp.get("method") == "notifications/message":
                continue
            if resp["id"] != req_id:
                # Out-of-order response for a different request — stash it.
    ...
    def handshake(
        ...
        """Выполнить MCP initialize + notifications/initialized с авто-определением протокола.

        Сначала пробуется JSON\\n (MCP SDK ≥ v1.28) с бюджетом probe_budget.
        При таймауте — откат на Content-Length (legacy), с оставшимся бюджетом.
        EOF/отмена → немедленное исключение без отката.
        """
        ...
        # Probe: пробуем newline-delimited (SDK ≥ v1.28) — 20% от бюджета, min 5s, max 10s.
        ...
        try:
            ...
            # Сервер ответил на JSON\n → протокол подтверждён.
            self._newline_proto = True
            logger.debug("MCP %s: protocol=newline-JSON (SDK ≥ v1.28)", self.name)
        except RuntimeError as exc:
            if "таймаут инициализации" not in str(exc):
                # EOF, отмена или другая реальная ошибка — не откатываемся.
                raise
            # Таймаут пробы → откат на Content-Length framing.
            logger.info(
