"""Workspace snapshot collection and input assembly.

Extracted from workspace_io.py: collect_workspace_snapshot,
collect_workspace_snapshot_async, _truncate_snapshot_to_fit,
...
def collect_workspace_snapshot(
    ...
    *,
    ...
    """Собрать текстовый снимок файлов для контекста LLM. Возвращает (текст, число файлов)."""
...
async def collect_workspace_snapshot_async(
    ...
    *,
    ...
    """Async wrapper — runs the blocking snapshot in a thread pool."""
    import asyncio
    import functools
...
def _truncate_snapshot_to_fit(
    ...
    """Trim file contents from the end of a snapshot block to fit within *budget_chars*.

    The directory tree header (lines before the first ``## file:`` entry) is
    always preserved.  File content blocks are dropped from the end until the
    section fits, or the budget is exhausted with only the header remaining.
    """
...
def build_input_with_workspace(
    ...
    *,
    ...
    """Порядок: manifest (стиль/архитектура) → снимок/индекс файлов → задача пользователя.

    After assembly the total length is checked against ``SWARM_INPUT_MAX_CHARS``
    (default 500 000 chars ≈ 167k tokens).  When exceeded, the workspace section
    (``# <workspace_section_title>``) is truncated from the end while
    ``# Project context`` and ``# User task`` are always preserved intact.
    """
