"""Workspace diff utilities — capture what the dev agent changed on disk."""
...
def capture_workspace_diff(workspace_root: Path, written_files: list[str]) -> dict[str, Any]:
    """Run a path-limited git diff in *workspace_root* and return structured diff data.

    Falls back to a file-list summary if git is unavailable or workspace is not a repo.

    Returns a dict with keys:
        diff_text: str — unified diff (empty string if git unavailable)
        files_changed: list[str] — relative paths of changed files
        stats: dict — {"added": int, "removed": int, "files": int}
        source: "git" | "file_list" — how the data was collected
    """
