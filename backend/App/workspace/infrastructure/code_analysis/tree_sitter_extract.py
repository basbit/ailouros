"""Извлечение сущностей через tree-sitter + tree-sitter-language-pack (опционально)."""
...
# Extend via env: SWARM_TREE_SITTER_GRAMMARS_EXTRA="swift:swift,csharp:c_sharp"
...
def _dedupe_entities(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for e in items:
        key = (e.get("kind"), e.get("name") or e.get("path"), e.get("line"))
        if key in seen:
            continue
        seen.add(key)
        out.append(e)
...
def _merge_route_heuristics(
    ...
    """Добавляем route/component из regex (scan) — импорт внутри, без цикла при загрузке модулей."""
    ...
    if swarm_lang in ("javascript", "typescript"):
        for e in _entities_js_like(text, rel, swarm_lang):
            if e.get("kind") in ("route", "component"):
                extra.append(e)
...
def extract_with_tree_sitter(
    ...
    *,
    ...
    """
    Возвращает {entities, grammar, parser} или None (fallback на старые экстракторы).
    """
    if disabled or not _pack_available():
        return None
    g = _grammar_name(swarm_lang, rel)
    if not g:
        return None
    ...
    try:
        if g == "python":
            ...
            return {"entities": entities, "grammar": g, "parser": "tree-sitter"}
        if g == "go":
        ...
        if g == "php":
            ...
            return {"entities": entities, "grammar": g, "parser": "tree-sitter"}
        elif g in ("javascript", "typescript", "tsx"):
            entities = _extract_js_ts(raw, rel, g)
            entities = _merge_route_heuristics(source, rel, swarm_lang, entities)
            entities = _dedupe_entities(entities)
            return {"entities": entities, "grammar": g, "parser": "tree-sitter"}
        elif g == "vue":
            ...
            script = m.group(1) if m else source
