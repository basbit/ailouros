"""P0.4: Structured defect model for QA/dev_review cycles.

Defects are the currency of the fix cycle: QA and dev_review produce them,
the orchestrator routes them back to dev until all P0/P1 are resolved.
"""
...
@dataclass
class Defect:
    """Structured defect produced by QA or dev_review."""
...
@dataclass
class DefectReport:
    """Collection of defects from a single QA or review pass."""
    ...
    def has_blockers(self) -> bool:
        """True if there are unresolved P0 or P1 defects."""
    ...
    def merge(self, other: DefectReport) -> None:
        """Merge another report into this one (for clustering)."""
...
def cluster_defects(defects: list[Defect]) -> dict[str, list[Defect]]:
    """P1.4: Group defects by category for batch fixing."""
...
def parse_defects_from_text(text: str) -> list[Defect]:
    """Best-effort extraction of defects from free-text QA/review output.

    Looks for structured blocks like:
        DEFECT: <title>
        SEVERITY: P0|P1|P2
        FILES: path1, path2
        EXPECTED: ...
...
def parse_defect_report(text: str) -> DefectReport:
    """Extract a structured defect report from text.

    Preferred format:
        <defect_report>{...json...}</defect_report>
    ...
    defects = parse_defects_from_text(text or "")
    if defects:
