"""WikiWriterAgent — LLM agent that generates structured wiki articles.

Activated by SWARM_WIKI_WRITER=1 (default off; falls back to stub generation
in wiki_auto_updater._write_session_article).

Model resolution:
    SWARM_MODEL_WIKI_WRITER → SWARM_MODEL (via resolve_agent_model).
...
def _restore_manual_blocks(generated: str, blocks: list[str]) -> str:
    """Append any preserved manual blocks at the end of the generated article."""
    if not blocks:
        return generated
    preserved = "\n\n".join(blocks)
...
class WikiWriterAgent(BaseAgent):
    """LLM agent that generates structured wiki articles from pipeline step output.

    Preserves sections marked <!-- manual -->...<!-- /manual --> in existing
    articles. Activated by SWARM_WIKI_WRITER=1 (default off).
    """
    ...
    def write_article(
        ...
        """Generate or update a wiki article. Returns a markdown string.

        Args:
            step_id: Pipeline step identifier (e.g. "session", "dev", "qa").
            step_output: Raw text output from the pipeline step (truncated to
                *max_chars* before being sent to the LLM).
            existing_content: Current article content, if any. Manual blocks
                inside are preserved verbatim in the output.
            max_chars: Maximum characters of step_output to include in the
                prompt. Defaults to 2000.

        Returns:
            Generated markdown string (with frontmatter).
        """
