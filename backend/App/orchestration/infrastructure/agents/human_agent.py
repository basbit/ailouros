"""Human approver — не вызывает LLM; фиксирует решение человека (или авто-апрув)."""
...
class HumanAgent:
    """Роль «человек»: апрув после ревью. По умолчанию авто-апрув для неинтерактивного режима."""
    ...
    def run(
        ...
        *,
        ...
        """context — объединённый вывод воркера + ревью.

        Args:
            wait_event: если задан, блокирует поток до его выставления вместо немедленного
                        HumanApprovalRequired.  Используется для интерактивных режимов.
            cancel_event: если выставлен до или во время ожидания — прерывает ожидание.
        """
        ...
        # Try in-process blocking approval (like shell_approval) if task context is available
