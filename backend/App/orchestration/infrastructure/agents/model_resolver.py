"""Pure routing helpers for agent model/URL resolution.

Extracted from ``base_agent.py`` — only reads env vars, no LLM calls.
"""
...
def resolve_model(role: str, role_default_model: str) -> str:
    ...
    Tiers: PM — coordinator (larger/thinking); BA+Architect — mid tier;
    Dev+QA — compact fast models.  ``SWARM_MODEL_PLANNING`` is no longer
    mixed into PM (only BA/ARCH/reviewers), so PM is not downgraded by a
    single variable.
    """
    role_key = role.upper()
    ...
    # SWARM_MODEL_PLANNING is not mixed into PM (see docstring).
...
def resolve_base_url(role: str) -> Optional[str]:
    """Return a role-specific base URL override, or None if not set.

    Reads ``SWARM_BASE_URL_<ROLE>`` then ``SWARM_BASE_URL``.
    Returns None when neither is set (caller falls back to environment/default).
    """
    role_key = role.upper()
