...
def resolve_default_environment() -> str:
    """Return the default LLM environment from SWARM_DEFAULT_ENVIRONMENT.

    Falls back to 'ollama' if not set (with a deprecation warning).
    """
...
class AgentModelResolver:
    """Thin wrapper around :func:`model_resolver.resolve_model` for DI purposes."""
...
def effective_cloud_provider(
    ...
    """Провайдер для cloud: явный из UI; иначе эвристика по имени модели.

    Не форсить Anthropic для gemini-моделей.
    """
...
# Project root resolved from SWARM_PROJECT_ROOT env var or by walking up from __file__
...
def load_prompt(prompt_relative_path: str, fallback: str) -> str:
    """Load prompt with overrides→upstream→fallback resolution.

    Resolution order:
    1. config/prompts/overrides/<prompt_relative_path>
    2. config/prompts/upstream/<prompt_relative_path>
    3. fallback string (hardcoded default)

    No silent fallback to var/ or other directories.
    """
    import logging
    ...
    # 3) direct path (backward compat for flat config/prompts/ layouts)
...
def resolve_agent_model(role: str, role_default_model: str = "") -> str:
    """Resolve model with local/cloud routing and role overrides.

    Resolution order (first match wins):
    1. SWARM_ROUTE_{ROLE} → cloud path → SWARM_MODEL_CLOUD_{ROLE} → SWARM_MODEL_CLOUD
    2. SWARM_MODEL_{ROLE} (per-role override)
    3. SWARM_MODEL_PLANNING / SWARM_MODEL_BUILD (tier group)
    4. SWARM_MODEL (global catch-all)
    5. role_default_model (deprecated — set SWARM_MODEL instead)

    If none are set → raises ValueError.
    """
    role_key = role.upper()
...
class BaseAgent:
    ...
    used_model: str = ""
    used_provider: str = ""
    # Из agent_config.remote_api (UI) или legacy cloud; пусто → env
    ...
    def run(self, user_input: str) -> str:
        ...
        # Always log estimated input size at DEBUG so operators can see it.
        ...
        # Derive log target from cfg for observability.
