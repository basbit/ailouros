"""LLM backend routing: maps (role, model, environment) to connection parameters.

Extracted from ``base_agent.py`` — no LLM calls, no client creation.
"""
...
class LLMBackendConfig:
    """Connection parameters for a single LLM backend call."""
...
def _effective_cloud_provider(
    ...
    """Resolve cloud provider: explicit from UI or heuristic from model name."""
...
class LLMBackendSelector:
    """Select LLM backend connection params for a given role/model/environment.

    Handles three routing cases:
    1. Local (ollama / lmstudio) — reads OPENAI_BASE_URL / LMSTUDIO_BASE_URL
    2. Cloud via Anthropic SDK — populates anthropic_* fields
    3. Cloud via OpenAI-compat API — populates base_url / api_key

    Does NOT create any client objects — only returns ``LLMBackendConfig``.
    """
    ...
    def select(
        ...
        """Return backend config for the given agent parameters.

        Args:
            role: Agent role (used for model/URL resolution from env vars).
            model: Resolved model name.
            environment: Backend type — "ollama", "lmstudio", "cloud", "anthropic".
            remote_provider: Optional explicit provider (from UI agent_config).
            remote_api_key: Optional explicit API key (from UI agent_config).
            remote_base_url: Optional explicit base URL (from UI agent_config).
            max_tokens: Optional max_tokens override (0 = use provider default).
            state: Optional pipeline state (reserved for future context-aware routing).

        Returns:
            LLMBackendConfig with all connection parameters populated.
        """
        env_key = (environment or "").lower()
    ...
    def ask_kwargs(self, cfg: LLMBackendConfig) -> dict[str, Any]:
        """Convert LLMBackendConfig into the kwargs dict expected by ``ask_model``."""
        kwargs: dict[str, Any] = {}
