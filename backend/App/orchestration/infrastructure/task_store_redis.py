"""TaskStoreRedis — infrastructure adapter implementing TaskStorePort.

Wraps the existing ``orchestrator.task_store.TaskStore`` without changing its
behaviour. Old modules are NOT removed (Strangler Fig pattern).
"""
...
class TaskStoreAdapter(TaskStorePort):
    """Adapter over ``orchestrator.task_store.TaskStore``.

    Args:
        store: An initialised ``TaskStore`` instance.
    """
    ...
    def create_task(self, task_id: TaskId, initial_data: dict[str, Any]) -> None:
        ...
        # If the caller supplied a specific task_id, we cannot override the
        # auto-generated one from the legacy store.  Log a warning when they differ.
