"""AgentToolBridge — generic "call agent X as tool" pattern (§12.3).

Enables agent-as-tool: Dev Lead spawns Dev sub-agents, QA calls Dev for fixes,
any agent calls Architect for clarification, PM calls BA for requirement details.

Usage (in a node function)::

    bridge = AgentToolBridge(state)
    result = bridge.call_agent("dev", subtask_spec)
    result = bridge.call_agent("architect", "Clarify authentication approach")
"""
...
# Cap sub-agent output to avoid bloating calling agent context
_BRIDGE_MAX_OUTPUT_CHARS = int(os.getenv("SWARM_AGENT_BRIDGE_MAX_OUTPUT_CHARS", "8000"))


class AgentToolBridge:
    """Spawns sub-agents on demand and returns their output as tool results.

    Designed to be used inside agentic tool loops (AgenticBaseAgent) or
    directly from node functions that need cross-agent calls.

    The bridge creates lightweight agent instances using the same agent_config
    as the main pipeline run so model/env settings are inherited.
    """
    ...
    def call_agent(
        ...
        *,
        ...
        """Spawn a sub-agent for *role* and return its response.

        Args:
            role: Agent role key (e.g. "dev", "architect", "ba", "pm").
            user_input: The task or question to send to the sub-agent.
            max_output_chars: Truncate output to this length before returning.

        Returns:
            The sub-agent's text output, truncated to *max_output_chars*.
        """
        import queue as _q
    ...
    def _run_agent(self, role: str, user_input: str) -> str:
        """Resolve and run the agent for *role* using pipeline agent_config."""
        state = self._state
    ...
    # ------------------------------------------------------------------
    # Convenience wrappers for common cross-agent patterns
    # ------------------------------------------------------------------

    def ask_architect(self, question: str) -> str:
        """Ask the Architect a clarifying question, enriched with current pipeline context."""
        state = self._state
        ctx_parts: list[str] = []
    ...
    def ask_ba(self, question: str) -> str:
        """Ask BA for requirement details, enriched with current pipeline context."""
        state = self._state
        ctx_parts: list[str] = []
    ...
    def execute_dev_subtask(self, spec: str) -> str:
        """Assign a coding subtask to a Dev agent and return its output."""
        return self.call_agent("dev", spec)

    def request_qa_verification(self, dev_output: str) -> str:
        """Ask QA to verify a dev output before proceeding."""
        return self.call_agent(
            "qa",
