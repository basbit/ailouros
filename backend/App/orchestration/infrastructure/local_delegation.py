"""Local (in-process) delegation adapter — R1.5 foundation.

Runs delegated sub-agent tasks synchronously in the same process.
Future implementations can use separate threads/processes/remote workers
while keeping the AgentDelegationPort interface unchanged.
"""
...
class LocalDelegationAdapter(AgentDelegationPort):
    """Runs delegated tasks in background threads with result tracking."""
    ...
    def _execute_role(self, request: DelegationRequest) -> str:
        """Execute via agent_factory if provided, else return a stub."""
