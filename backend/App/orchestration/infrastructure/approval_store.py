"""Redis-backed approval store for shell and human gates.

Pending approval data is persisted in Redis so it survives process restarts
and is visible across workers. The blocking wait mechanism still uses
in-process threading.Event (pipeline thread blocks locally).

When Redis is unavailable, falls back to in-process dicts with a warning.

Keys:
    swarm:approval:shell:{task_id}  — JSON list of shell commands
    swarm:approval:human:{task_id}  — JSON string of human context
    swarm:approval:result:{task_id} — JSON {approved: bool, user_input: str}
"""
...
def store_pending(gate_type: str, task_id: str, data: Any) -> None:
    """Store pending approval data (shell commands or human context)."""
...
def load_pending(gate_type: str, task_id: str) -> Optional[Any]:
    """Load pending approval data. Returns None if not found."""
    key = f"swarm:approval:{gate_type}:{task_id}"
    r = _redis()
    if r is not None:
        try:
...
def store_result(task_id: str, approved: bool, user_input: str = "") -> None:
    """Store the approval result (used by HTTP handler)."""
...
def load_result(task_id: str) -> Optional[dict[str, Any]]:
    """Load approval result. Returns None if not set yet."""
    key = f"swarm:approval:result:{task_id}"
    r = _redis()
    if r is not None:
        try:
...
def clear_result(task_id: str) -> None:
    """Remove approval result after consumption."""
    key = f"swarm:approval:result:{task_id}"
    r = _redis()
    if r is not None:
        try:
