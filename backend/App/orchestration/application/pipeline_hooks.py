"""Опциональные хуки до/после шага пайплайна (extension points).

Модуль задаётся:
- ``SWARM_PIPELINE_HOOKS_MODULE`` (dotted path, напр. ``my_pkg.swarm_hooks``), или
- ``agent_config.swarm.pipeline_hooks_module`` (имеет приоритет над env).

Ожидаемые функции (любая может отсутствовать):

- ``before_pipeline_step(step_id, state) -> dict | None`` — optional partial state
  merge **before** the node runs.

- ``after_pipeline_step(step_id, state, step_delta) -> None`` — ``state`` is post-step;
  ``step_delta`` is the dict the node just returned.
"""
