"""PipelineStepRegistry — owns PIPELINE_STEP_SEQUENCE and PIPELINE_STEP_REGISTRY.

Extracted from ``pipeline_graph.py``.  ``pipeline_graph.py`` imports these
names from here and re-exports them for backward compat.
"""
...
# Backward compatibility with old pipeline_steps / UI
...
class PipelineStepRegistry:
    """Wraps PIPELINE_STEP_SEQUENCE and PIPELINE_STEP_REGISTRY with resolve/validate methods."""

    def __init__(self) -> None:
        self._registry = PIPELINE_STEP_REGISTRY

    def resolve(self, step_name: str) -> Callable[[PipelineState], dict[str, Any]]:
        """Return the node function for *step_name*.
    ...
    def validate(self, steps: list[str]) -> None:
        """Raise ValueError if any step in *steps* is not registered.

        Args:
            steps: List of step_id strings to validate.

        Raises:
            ValueError: if *steps* is empty or contains unknown step ids.
        """
        if not steps:
...
def validate_pipeline_steps(
    ...
    """Module-level convenience function using PipelineStepRegistry.

    Also supports custom role steps (``crole_<slug>``).  agent_config is
    needed to resolve custom roles.
    """
    if not steps:
        raise ValueError("pipeline_steps must be a non-empty list")
