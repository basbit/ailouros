...
def source_research_needed(state: PipelineState) -> bool:
    ...
    if isinstance(agent_config, dict):
        ...
        if isinstance(swarm_cfg, dict) and "require_source_research" in swarm_cfg:
            return is_truthy_value(swarm_cfg.get("require_source_research"), default=False)
...
def _tool_fetch_page_factory(state: PipelineState):
    quarantine = QuarantineAgent(state=ephemeral_as_dict(state))
    ...
    *,
    ...
    if not source_research_needed(state):
        if not str(state.get("source_research_output") or "").strip():
            ...
            state["research_manifest"] = {
                "schema": "swarm_research_manifest/v1",
                "status": "not_required",
                "required": False,
                "queries": [],
                "sources": [],
    ...
    try:
        ...
        state["research_manifest"] = {
            "schema": "swarm_research_manifest/v1",
            "status": "completed" if payload else "unstructured",
            "required": True,
            "queries": list(payload.get("queries") or []) if payload else [query],
            "sources": list(payload.get("sources") or []) if payload else [],
            "summary": str(payload.get("summary") or "") if payload else "",
            "unavailable_reason": "",
        }
