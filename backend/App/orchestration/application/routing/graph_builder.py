...
from backend.App.orchestration.application.pipeline.step_decorator import hook_wrap as _raw_hook_wrap
...
def _planning_review_gate_wrapper(
    ...
    output_key = critical_review_step_to_output_key().get(step_id, f"{step_id}_output")
...
def _verification_layer_node(state: PipelineState) -> dict[str, Any]:
    ...
    if "workspace_writes" in state:
        updates["workspace_writes"] = get_ephemeral(state, "workspace_writes")
    if gate_results:
    ...
    def build(self):
        ...
        definition = GraphDefinition(
            nodes=[
                ...
                NodeDef("REVIEW_PM", hook_wrap("review_pm", _planning_review_gate_wrapper("review_pm", review_pm_node))),
                ...
                NodeDef("REVIEW_STACK", hook_wrap("review_stack", _planning_review_gate_wrapper("review_stack", review_stack_node))),
                NodeDef("REVIEW_ARCH", hook_wrap("review_arch", _planning_review_gate_wrapper("review_arch", review_arch_node))),
