...
def run_pipeline_stream_retry(
    ...
    restart_agent_config_raw = override_agent_config if isinstance(override_agent_config, dict) and override_agent_config else state.get("agent_config")
    restart_agent_config: dict[str, Any] = restart_agent_config_raw if isinstance(restart_agent_config_raw, dict) else {}
    topology = topology_from_agent_config(restart_agent_config)
    ring_pass_value = int(cast(Any, state.get("_ring_pass")) or 0)
    ring_evaluation = evaluate_ring_restart(
        cast(dict[str, Any], state), topology, pipeline_steps, ring_pass_value,
    )
    if ring_evaluation["should_restart"]:
        ...
        ring_final: PipelineState = yield from run_pipeline_stream(
            ...
            agent_config=restart_agent_config,
