...
def _initial_pipeline_state(
    ...
    *,
    ...
    task_contract = {
        ...
        "workspace_write_expected": bool(workspace_apply_writes),
        "workspace_root": workspace_root,
        "pipeline_steps": list(pipeline_step_ids or []),
        "constraints_source": "original_user_prompt",
    }
    ...
    initial_state["research_manifest"] = {}
    ...
    initial_state["devops_execution_contract"] = {}
    ...
    initial_state["workspace_writes"] = {}
    initial_state["workspace_writes_incremental"] = []
    initial_state["workspace_truth"] = {}
    ...
    _set_feature_env(swarm_cfg, "self_verify_provider", "SWARM_SELF_VERIFY_PROVIDER", is_str=True)
    ...
    _set_feature_env(swarm_cfg, "deep_planning_provider", "SWARM_DEEP_PLANNING_PROVIDER", is_str=True)
    ...
    _set_feature_env(swarm_cfg, "background_agent_provider", "SWARM_BACKGROUND_AGENT_PROVIDER", is_str=True)
    _set_feature_env(swarm_cfg, "background_agent_model", "SWARM_BACKGROUND_AGENT_MODEL", is_str=True)
...
_COMPACTION_KEEP_KEYS: frozenset[str] = frozenset({
    "input", "user_task", "task_contract", "raw_user_task",
    "project_manifest", "workspace_context_mode",
    ...
    "planning_review_feedback", "planning_review_blockers",
...
def _compact_state_if_needed(
    ...
    from backend.App.orchestration.application.context.state_summarizer import (
    ...
    for key in sorted(_COMPACTION_SUMMARISE_KEYS):
        ...
        if isinstance(val, str) and len(val) > 200:
            if _use_llm_summary:
                ...
                state[key] = _bulletpoint_compact(val)
            dropped.append(key)
