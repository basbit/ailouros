...
def ba_arch_debate_node(state: PipelineState) -> dict[str, Any]:
    """DebateWithJudge: Judge resolves conflicts between BA and Architect.

    Optional step — add "ba_arch_debate" to pipeline_steps between human_arch and spec_merge.
    Identifies requirement/architecture inconsistencies and proposes compromises.
    Result is automatically included in spec_merge.
    """
...
def merge_spec_node(state: PipelineState) -> dict[str, Any]:
    ...
    # S4 gate: abort if both critical upstream steps produced empty/trivial output.
    # One empty input is tolerated (e.g. pipeline without BA step); both empty means
    # upstream agents failed silently — producing a spec from nothing is pointless.
