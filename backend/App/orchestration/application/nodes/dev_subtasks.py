"""Dev/QA subtask plan helpers.

Extracted from dev.py: parse_dev_qa_task_plan, read_dev_qa_task_count_target,
normalize_dev_qa_tasks_to_count, _dev_spec_max_chars, _dev_devops_max_chars.
"""
...
def _extract_json_by_brackets(text: str) -> list[str]:
    """Scan *text* and extract all top-level JSON objects / arrays using
    bracket counting.  Returns candidates ordered longest-first so the most
    complete parse wins.

    This is a fallback for when the LLM wraps JSON in prose and the code-block
    regex fails to capture the balanced closing bracket.
    """
    results: list[str] = []
...
def parse_dev_lead_plan(raw: str) -> dict[str, Any]:
    """Extract canonical Dev Lead plan artifact from output.
    ...
    ```json
    {
      ...
      "deliverables": {
        "must_exist_files": [...],
        "spec_symbols": [...],
        "verification_commands": [{"command":"...", "expected":"..."}],
        "assumptions": ["..."]
      }
    }
    ```
    Backward-compatible fallback: a plain JSON task array.
    """
    text = (raw or "").strip()
    if not text:
        return {"tasks": [], "deliverables": _normalize_deliverables(None), "has_deliverables": False}

    # Greedy match so we capture up to the LAST } / ] before the closing ```
    # (lazy *? stops at the first closing bracket — breaks nested JSON objects)
    blocks = re.findall(
        r"```(?:json)?\s*([\[{][\s\S]*[\]}])\s*```",
        text,
        flags=re.IGNORECASE,
...
def parse_dev_qa_task_plan(raw: str) -> list[dict[str, Any]]:
    """Extract JSON subtask array from Dev Lead output."""
...
def read_dev_qa_task_count_target(
    ...
    """Number of Dev+QA subtask pairs after dev_lead.

    Priority: ``swarm.dev_qa_task_count`` (legacy) →
    ``max(swarm.dev_task_count, swarm.qa_task_count)`` →
    env ``SWARM_DEV_QA_TASK_COUNT``.
    """
    ac = agent_config or {}
    swarm = ac.get("swarm")
...
def normalize_dev_qa_tasks_to_count(
    ...
    """Trim or pad task plan to exactly `target` subtasks."""
    if target < 1:
        return list(tasks)
...
def _dev_spec_max_chars() -> int:
    """Max chars for the full specification block sent to Dev.

    Default 80 000 chars ≈ 20 000 tokens.
    Override via SWARM_DEV_SPEC_MAX_CHARS env var.
    """
...
def _dev_devops_max_chars() -> int:
    """Max chars for the DevOps context block sent to Dev.

    Default 20 000 chars ≈ 5 000 tokens.
    Override via SWARM_DEV_DEVOPS_MAX_CHARS env var.
    """
