...
def _qa_dev_output_max_chars() -> int:
    """Max chars for the Dev output slice passed to QA per subtask.

    Default 80 000 chars ≈ 20 000 tokens — enough for one subtask's code output.
    Override via SWARM_QA_DEV_OUTPUT_MAX_CHARS env var.
    """
...
def _review_dev_output_max_chars() -> int:
    """Max chars for the dev_output block embedded in the QA reviewer prompt.

    Mirrors the same cap used in review_dev_node.
    Override via SWARM_REVIEW_DEV_OUTPUT_MAX_CHARS env var (default 60_000).
    """
...
def _review_spec_max_chars() -> int:
    """Max chars for the qa_output block embedded in the QA reviewer prompt.

    Reuses the SWARM_REVIEW_SPEC_MAX_CHARS env var (default 40_000) to apply a
    consistent cap on the second large artifact in the reviewer prompt.
    """
