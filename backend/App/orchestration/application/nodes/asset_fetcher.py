...
def _make_asset_fetcher_agent(state: PipelineState) -> BaseAgent:
    ...
    if prompt_text_override:
        ...
        system_prompt = load_prompt(
            ...
            fallback=_prompt_fragment("asset_fetcher_system_prompt_fallback"),
        )
    else:
        system_prompt = _prompt_fragment("asset_fetcher_system_prompt")
    skills_extra = _skills_extra_for_role_cfg(state, cfg)
    if skills_extra:
        system_prompt = system_prompt.rstrip() + "\n\n" + skills_extra
    model_value = (
        _cfg_model(cfg)
        or os.getenv("SWARM_ASSET_FETCHER_MODEL", "").strip()
        or resolve_agent_model("ASSET_FETCHER")
    )
