"""Structured repo-evidence parsing and workspace validation helpers."""
...
def parse_repo_evidence_artifact(raw_output: str) -> dict[str, Any]:
    """Parse a canonical repo-evidence artifact from agent output.
      ...
      "repo_evidence": [
        {
          ...
          "start_line": 10,
          "end_line": 20,
          "excerpt": "text copied from file",
          "why": "why it proves the claim"
        }
      ],
      "unverified_claims": ["..."]
    }
...
def validate_repo_evidence_against_workspace(
    ...
    *,
    ...
    """Validate repo_evidence excerpts against workspace contents."""
...
def enforce_repo_evidence_policy(
    ...
    *,
    ...
    """Return only validated repo-backed evidence.

    When workspace access is available, unresolved repo-based claims are not accepted as
    validated evidence. Instead of hard-failing the whole step, we suppress them from the
    canonical artifact and keep them only as diagnostic metadata. This keeps the artifact
    evidence-first while avoiding brittle failures on model-side over-reporting.
    """
...
def ensure_validated_repo_evidence(
    *,
    ...
    """Validate repo_evidence and repair it across all evidence-backed steps.

    Flow:
    1. Validate the original answer.
