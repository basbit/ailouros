"""Gate orchestration (application layer).

I/O-bound runners that collect evidence from infrastructure adapters
and delegate to pure domain evaluators in :mod:`domain.gates`.

Direction of dependencies (DDD §10.1):
    application → domain  (evaluator functions, dataclasses)
    application → infrastructure (evidence collection)

The domain layer remains pure (no infrastructure imports).
"""
...
def run_build_gate(
    ...
    """Discover and execute trusted build commands.  Returns gate result."""
    ...
    for cmd in commands:
        try:
            ...
            # Explicit: command binary missing → record diagnostic, do not
            # silently skip (§2 fail-explicit).
            logger.info(
                "build_gate: skipped command %s (binary not on PATH): %s",
                cmd[0], exc,
            )
            continue
...
def run_spec_gate(
    ...
    """Collect path-existence evidence and evaluate spec gate."""
...
def run_consistency_gate(
    ...
    """Collect symbol-presence and namespace evidence; evaluate."""
...
def detect_stubs(
    ...
    """Scan workspace for stub patterns and return findings."""
...
# Tokens for runtime-driven detection (kept here in application layer because
# they describe how to inspect manifest content, not domain rules).
...
def run_diff_risk_gate(
    ...
    """Inspect git diff + manifest writes; build evidence; evaluate."""
    errors: list[dict[str, Any]] = []
    ...
    # Inspect git diff for large deletions
    ...
    try:
        ...
        # Git not on PATH: explicit log, no fallback into "best-effort" logic (§2).
...
def run_all_gates(
    ...
    """Run all verification gates in canonical order."""
    return [
