"""Tests for pipeline_step_runner.py — _emit_completed, primary_output_for_step,
final_pipeline_user_message, task_store_agent_label, _format_elapsed_wall,
...
def test_emit_completed_custom_role():
    ...
    ev = _emit_completed("crole_my_agent", state)
...
def test_run_step_with_stream_progress_basic():
    ...
    with patch(
        "backend.App.orchestration.application.pipeline_step_runner._pipeline_should_cancel",
        return_value=False,
    ), patch(
        "backend.App.orchestration.application.pipeline_step_runner._stream_progress_heartbeat_seconds",
        return_value=100.0,
    ):
        list(_run_step_with_stream_progress("pm", step_fn, state))
...
def test_run_step_with_stream_progress_exception_propagated():
    ...
    with patch(
        ...
        with pytest.raises(RuntimeError, match="step crashed"):
            list(_run_step_with_stream_progress("pm", failing_step, state))
...
def test_run_step_with_stream_progress_cancel_raises():
    ...
    with patch(
        ...
        with pytest.raises(PipelineCancelled):
            list(_run_step_with_stream_progress("pm", step_fn, {}))
