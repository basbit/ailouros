...
def test_real_provider_writes_vector_alongside_pattern(
    real_embedding_provider, tmp_path
):
    path = tmp_path / "patterns.json"
    store_pattern(
        path, "default", "auth-jwt", "Use JWT with refresh tokens", merge=False
    )
    import json
...
def test_semantic_match_beats_token_disjoint_competitor(
    real_embedding_provider, tmp_path
):
    path = tmp_path / "patterns.json"
    store_pattern(
...
def test_semantic_score_strictly_higher_than_token_only(
    real_embedding_provider, tmp_path, monkeypatch
):
