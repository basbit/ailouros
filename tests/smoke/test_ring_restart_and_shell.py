...
class TestRingDefectContext:
    def test_includes_all_defects(self):
        ...
        ctx = build_ring_pass_defect_context(defects, 1)
    ...
    def test_pass_number_in_header(self):
        ...
        ctx = build_ring_pass_defect_context(
            [{"severity": "P0", "description": "Bug"}], 3
        )
    ...
    def test_no_defects_produces_valid_block(self):
        ...
        ctx = build_ring_pass_defect_context([], 2)
        assert "Ring pass 2" in ctx


class TestRingRestartConditions:
    def _make_ring_restart_generator(
        self, monkeypatch, *, open_defects, topology, ring_max="1"
    ):
        ...
        mock_extr = MagicMock()
        mock_extr.emit_completed = MagicMock(
            return_value={"agent": "pm", "status": "completed", "message": ""}
        )
        ...
        patches = [
            ...
            patch(
                "backend.App.orchestration.application.pipeline.pipeline_runners.reset_pipeline_machine"
            ),
            patch(
                "backend.App.orchestration.application.pipeline.pipeline_runners.get_pipeline_machine",
                return_value=MagicMock(),
            ),
            patch(
                "backend.App.orchestration.application.pipeline.pipeline_runners.sync_pipeline_machine"
            ),
            patch(
                "backend.App.orchestration.application.pipeline.pipeline_runners.prepare_pipeline_machine_for_step"
            ),
            patch(
                "backend.App.orchestration.application.pipeline.pipeline_runners.finalize_pipeline_machine"
            ),
            patch(
                "backend.App.orchestration.application.pipeline.pipeline_runners._run_post_step_enforcement",
                return_value=iter([]),
            ),
            patch(
                "backend.App.orchestration.application.pipeline.pipeline_runners._step_executor",
                mock_exec,
            ),
            patch(
                "backend.App.orchestration.application.pipeline.pipeline_runners._step_extractor",
                mock_extr,
            ),
        ]
    ...
    def test_ring_restart_does_not_fire_without_ring_topology(self, monkeypatch):
        ...
        ring_events = [e for e in events if e.get("status") == "ring_restart"]
        assert not ring_events, (
            f"Unexpected ring_restart for linear topology: {ring_events}"
        )
    ...
    def test_ring_restart_respects_max_restarts_zero(self, monkeypatch):
        ...
        ring_events = [e for e in events if e.get("status") == "ring_restart"]
        assert not ring_events, (
            f"Unexpected ring_restart when SWARM_RING_MAX_RESTARTS=0: {ring_events}"
        )
    ...
    def test_ring_restart_no_fire_when_no_defects(self, monkeypatch):
        ...
        ring_events = [e for e in events if e.get("status") == "ring_restart"]
        assert not ring_events, (
            f"Unexpected ring_restart when no defects: {ring_events}"
        )
...
class TestShellApprovalFlow:
    ...
    def test_out_of_allowlist_command_is_blocked(self, monkeypatch):
        ...
        assert "godot" not in allowlist, (
            f"Test assumes godot NOT in allowlist, got {allowlist}"
        )

        with scoped_runtime_shell_allowlist():
            allowed, reason = _shell_command_allowed("godot --headless project.tscn")
            assert not allowed, (
                f"godot should be blocked before approval, reason: {reason}"
            )
    ...
    def test_runtime_allowlist_extension_permits_subsequent_run(self, monkeypatch):
        ...
        with scoped_runtime_shell_allowlist():
            ...
            assert allowed_after, (
                "godot must be allowed after extend_runtime_shell_allowlist"
            )

        with scoped_runtime_shell_allowlist():
            allowed_new_scope, _ = _shell_command_allowed(
                "godot --headless project.tscn"
            )
