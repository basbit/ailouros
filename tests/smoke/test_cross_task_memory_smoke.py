"""Real-model smoke tests for the semantic layer of ``cross_task_memory``.

Verifies that with a real sentence-transformers provider, hybrid ranking
of replayed episodes finds semantically related runs even when the
query has no token overlap with the stored body — the case the legacy
token-only scorer would silently miss.
"""
...
def test_paraphrased_query_finds_correct_episode(real_embedding_provider):
    """Query has zero meaningful token overlap with any stored body —
    only the semantic layer can rank the right episode first."""
    state = {}
    with patch(
...
def test_semantic_finds_episode_token_search_completely_misses(
    ...
    """Token-only search returns nothing for a fully-paraphrased query.
    Semantic search must still surface the right episode at rank 1."""
    ...
    with patch(
        ...
        # No meaningful token overlap with either body (3+ char tokens):
        # query tokens = {how, accelerate, frequently, requested, lookups}
        query = "how to accelerate frequently requested lookups"

        # Token-only baseline: should find no hits.
        ...
        # Semantic mode: should rank the redis episode first.
        monkeypatch.delenv("SWARM_CROSS_TASK_MEMORY_SEMANTIC", raising=False)
        embedding_service.reset_embedding_provider()
