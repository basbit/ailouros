...
def test_apply_workspace_pipeline_collects_asset_request(tmp_path):
    text = (
        '<swarm_asset_request path="Assets/Art/JournalSkin.png" '
        'source="generate" prompt="painted journal cover" provenance="user_prompt" />'
    )

    result = apply_workspace_pipeline(text, tmp_path)

    assert result["asset_requests"][0]["path"] == "Assets/Art/JournalSkin.png"
    assert result["asset_requests"][0]["source"] == "generate"
    assert "Assets/Art/JournalSkin.png" in result["binary_assets_requested"]
    assert result["written"] == []
    assert result["errors"] == []
