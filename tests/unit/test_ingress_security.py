...
from backend.App.shared.infrastructure.rest.utils import _chat_sync_prepare_workspace_and_task
...
def test_rewrite_untrusted_input_parses_agent_output() -> None:
    ...
    with patch(
        "backend.App.orchestration.application.enforcement.ingress_security.run_agent_with_boundary",
...
def test_chat_prepare_workspace_uses_rewritten_prompt_but_keeps_raw_task() -> None:
    ...
    with patch(
        "backend.App.orchestration.application.enforcement.ingress_security.rewrite_untrusted_input",
        return_value=rewrite,
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.prepare_workspace",
