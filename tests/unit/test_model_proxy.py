...
def test_format_context_window_millions():
    assert _format_context_window(2_000_000) == "2M ctx"


def test_format_context_window_thousands():
    assert _format_context_window(131_072) == "131k ctx"


def test_format_context_window_small():
    assert _format_context_window(512) == "512 ctx"


def test_format_context_window_none():
    assert _format_context_window(None) == ""


def test_format_context_window_invalid_string():
    assert _format_context_window("not-a-number") == ""


def test_format_context_window_zero():
    assert _format_context_window(0) == "0 ctx"


def test_format_context_window_exactly_1000():
    assert _format_context_window(1000) == "1k ctx"


def test_format_context_window_string_number():
    # Should handle string-encoded ints
    result = _format_context_window("8192")
    assert "8k" in result
