...
def test_entities_csharp_unity_symbols():
    source = """
namespace Garden.Gameplay
{
    public partial class JournalController : MonoBehaviour
    {
        [SerializeField] private Sprite journalSkin;

        private void Awake()
        {
            if (journalSkin == null) return;
        }
...
"""
    entities = _entities_csharp(source, "Assets/Scripts/JournalController.cs")
    names_by_kind = {(entity["kind"], entity["name"]) for entity in entities}

    assert ("namespace", "Garden.Gameplay") in names_by_kind
    assert ("class", "JournalController") in names_by_kind
    assert ("field", "journalSkin") in names_by_kind
    assert ("method", "Awake") in names_by_kind
    assert ("method", "OpenJournal") in names_by_kind
    assert ("method", "if") not in names_by_kind
...
def test_extract_file_csharp(tmp_path):
    source_file = tmp_path / "PlayerController.cs"
    source_file.write_text("public class PlayerController : MonoBehaviour { public void Start() {} }")
    with patch(
        "backend.App.workspace.infrastructure.code_analysis.scan.extract_with_tree_sitter",
        return_value=None,
    ):
        result = _extract_file(source_file, "Assets/Scripts/PlayerController.cs", "csharp", tree_sitter_disabled=True)
    assert result["language"] == "csharp"
    assert any(entity["name"] == "PlayerController" for entity in result["entities"])
