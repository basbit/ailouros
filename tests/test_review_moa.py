...
def test_truthy_integer():
    assert _truthy(1) is False  # Only True bool is truthy
...
def test_with_semaphore_releases_on_exception():
    from backend.App.orchestration.application.review_moa import _parallel_semaphore
...
def test_run_reviewer_or_moa_moa_enabled():
    ...
    with patch(
        "backend.App.orchestration.application.review_moa.swarm_max_parallel_tasks",
        return_value=4,
    ):
        result = run_reviewer_or_moa(
            state,
