...
def test_truthy(val, expected):
    ...
    assert is_truthy_value(val) == expected
...
def test_memory_configuration_returns_config():
    state = _state(agent_config={"swarm": {"cross_task_memory": {"enabled": True}}})
    assert _memory_configuration(state) == {"enabled": True}


def test_memory_configuration_non_dict_returns_empty():
    state = _state(agent_config={"swarm": {"cross_task_memory": "yes"}})
    assert _memory_configuration(state) == {}
