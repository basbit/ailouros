...
FRONTEND_SHELL_SCOPE = [
    ...
    REPO_ROOT / "frontend/src/shared/components/HostMetrics.vue",
    REPO_ROOT / "frontend/src/widgets/task-monitor/TaskMonitor.vue",
    REPO_ROOT / "frontend/src/features/project-settings/ProjectSwitcher.vue",
...
def _template_lines(path: Path) -> list[tuple[int, str]]:
    # Frontend submodule may be at a commit where some of the hard-coded
    # paths below don't exist; treat missing files as "no lines to check"
    # rather than crashing the whole test.
    if not path.is_file():
        return []
...
def test_frontend_script_i18n_scope_has_no_known_raw_ui_literals() -> None:
    ...
    for path in FRONTEND_SCRIPT_I18N_SCOPE:
        ...
        for snippet in RAW_UI_LITERAL_SNIPPETS:
            # Require the snippet to appear inside a quoted string so we don't
            # match identifiers like ``onArtifactsToggle`` when the literal is
            # e.g. ``"Artifacts"``.
            if (
                f'"{snippet}"' in text
                or f"'{snippet}'" in text
                or f"`{snippet}`" in text
            ):
