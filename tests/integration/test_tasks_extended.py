...
def test_resolve_chat_request_basic():
    ...
    with patch(
        "backend.App.orchestration.application.use_cases.tasks.merge_agent_config",
        return_value={"merged": True},
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.resolve_preset",
        return_value=["x"],
    ), patch(
        "backend.App.integrations.application.user_settings_service.get_persisted_automation_settings",
        return_value={},
    ):
        agent_config, steps = resolve_chat_request(req)

    # resolve_chat_request folds global automation settings into agent_config["swarm"].
    # With no persisted settings, the "swarm" key is still created (empty) as a side
    # effect of _apply_global_automation_settings.
    assert agent_config.get("merged") is True
    assert steps == ["pm", "ba"]
...
def test_prepare_workspace_no_root():
    with patch(
        "backend.App.orchestration.application.use_cases.tasks.resolve_workspace_context_mode",
        return_value="full",
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.build_input_with_workspace",
...
def test_prepare_workspace_workspace_write_without_env(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "backend.App.orchestration.application.use_cases.tasks.workspace_write_allowed",
        return_value=False,
    ):
...
def test_prepare_workspace_with_project_context_file(tmp_path):
    ...
    with patch(
        ...
        "backend.App.orchestration.application.use_cases.tasks.validate_workspace_root",
        return_value=tmp_path.resolve(),
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.resolve_project_context_path",
        return_value=ctx_file,
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.validate_readable_file",
        return_value=ctx_file,
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.read_project_context_file",
        return_value="project context data",
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.collect_workspace_snapshot",
...
def test_prepare_workspace_tools_only_no_root():
    with patch(
        ...
        "backend.App.orchestration.application.use_cases.tasks.tools_only_workspace_placeholder",
...
def test_prepare_workspace_index_only_mode(tmp_path):
    with patch(
        ...
        "backend.App.orchestration.application.use_cases.tasks.collect_workspace_file_index",
...
def test_start_pipeline_run_success(tmp_path):
    ...
    with patch(
        ...
        "backend.App.orchestration.application.use_cases.tasks.run_pipeline",
        return_value=mock_result,
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.final_pipeline_user_message",
        return_value="final message",
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.task_store_agent_label",
        return_value="dev",
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.workspace_write_allowed",
        return_value=False,
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.ARTIFACT_AGENT_OUTPUT_KEYS",
        [("pm", "pm_output"), ("dev", "dev_output")],
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.append_task_run_log",
    ):
...
def test_prepare_workspace_retrieve_mode_with_mcp_servers(tmp_path):
    ...
    with patch(
        ...
        "backend.App.orchestration.application.use_cases.tasks.apply_auto_mcp_to_agent_config",
...
def test_prepare_workspace_priority_paths_mode(tmp_path):
    ...
    with patch(
        ...
        "backend.App.orchestration.application.use_cases.tasks.collect_workspace_priority_snapshot",
...
def test_start_pipeline_run_workspace_writes_applied(tmp_path):
    ...
    with patch(
        ...
        "backend.App.orchestration.application.use_cases.tasks.run_shell_after_user_approval",
        return_value=MagicMock(),
    ), patch(
        "backend.App.orchestration.application.use_cases.tasks.apply_from_devops_and_dev_outputs",
