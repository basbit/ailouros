...
def test_pipeline_happy_path(monkeypatch):
    ...
    monkeypatch.setattr("backend.App.orchestration.application.pipeline_graph.CodeDiagramAgent.run", lambda self, _: "mermaid ok")
    monkeypatch.setattr("backend.App.orchestration.application.pipeline_graph.DocGenerateAgent.run", lambda self, _: "docs ok")
    monkeypatch.setattr(
        "backend.App.orchestration.application.pipeline_graph.ProblemSpotterAgent.run",
        lambda self, _: (
            "problems ok\n```json\n"
            '{"repo_evidence":[],"unverified_claims":["Repository facts unavailable in this test."]}\n'
            "```"
    ...
    monkeypatch.setattr(
        "backend.App.orchestration.application.pipeline_graph.RefactorPlanAgent.run",
        lambda self, _: (
            "refactor plan ok\n```json\n"
            '{"repo_evidence":[],"unverified_claims":["Repository facts unavailable in this test."]}\n'
            "```"
